...
public class PlayerController : MonoBehaviour {
    ...
    ///*
    // MOBILE INPUT
    // Uncomment this complete Update() function to enable mobile controls. But comment out the whole Update() funtion below this one.
    ...
    void Update () {
        if (ControlManager.instance.mobile)
        {
            if (canMove)
            {
                rigidBody.velocity = new Vector2(Mathf.RoundToInt(CF2Input.GetAxis("Horizontal")), Mathf.RoundToInt(CF2Input.GetAxis("Vertical"))) * moveSpeed;
            }
            else
            {
            ...
            if (CF2Input.GetAxisRaw("Horizontal") == 1 || CF2Input.GetAxisRaw("Horizontal") == -1 || CF2Input.GetAxisRaw("Vertical") == 1 || CF2Input.GetAxisRaw("Vertical") == -1)
            {
                if (canMove)
                {
                    Animation.SetFloat("lastMoveX", CF2Input.GetAxisRaw("Horizontal"));
                    Animation.SetFloat("lastMoveY", CF2Input.GetAxisRaw("Vertical"));
                }
            }
