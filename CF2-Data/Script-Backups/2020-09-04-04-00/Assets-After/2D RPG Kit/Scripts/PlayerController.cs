...
public class PlayerController : MonoBehaviour {
    ...
    void Update () {
        if (ControlManager.instance.mobile)
        {
            if (canMove)
            {
                rigidBody.velocity = new Vector2(Mathf.RoundToInt(ControlFreak2.CF2Input.GetAxis("Horizontal")), Mathf.RoundToInt(ControlFreak2.CF2Input.GetAxis("Vertical"))) * moveSpeed;
            }
            else
            {
            ...
            if (ControlFreak2.CF2Input.GetAxisRaw("Horizontal") == 1 || ControlFreak2.CF2Input.GetAxisRaw("Horizontal") == -1 || ControlFreak2.CF2Input.GetAxisRaw("Vertical") == 1 || ControlFreak2.CF2Input.GetAxisRaw("Vertical") == -1)
            {
                if (canMove)
                {
                    Animation.SetFloat("lastMoveX", ControlFreak2.CF2Input.GetAxisRaw("Horizontal"));
                    Animation.SetFloat("lastMoveY", ControlFreak2.CF2Input.GetAxisRaw("Vertical"));
                }
            }
        ...
        if (!ControlManager.instance.mobile)
        {
            if (canMove)
            {
                rigidBody.velocity = new Vector2(ControlFreak2.CF2Input.GetAxisRaw("Horizontal"), ControlFreak2.CF2Input.GetAxisRaw("Vertical"));
